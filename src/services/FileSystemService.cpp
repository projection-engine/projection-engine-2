...
namespace PEngine {
    std::string FileSystemService::SEP = "\\";

    void FileSystemService::WriteFile(const std::string &filePath, const std::string &data) {
    ...
    void FileSystemService::DeleteFileOrDir(const std::string &filePath) {
    ...
    std::string FileSystemService::GetCurrentPath() {
        return std::filesystem::current_path().string();
    }

    std::string FileSystemService::ReadFile(const std::string &filePath) {
        if (std::filesystem::exists(filePath)) {
    ...
    bool FileSystemService::CreateDir(const std::string &path) {
        if (!Exists(path)) {
            try {
    ...
    std::string FileSystemService::GetRootDir() {
        try {
    ...
    std::string FileSystemService::ReadDirectory(const std::string& directoryPath) {
    ...
    void FileSystemService::HandleEvent(WebViewPayload &payload) {
        payload.webview->getLogger()->info("HANDLING FileSystemService EVENT");
