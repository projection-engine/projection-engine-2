export default class ArrayBufferAPI {
	static allocateVector(size:number, defaultValue?:number, asQuaternion?:boolean, shared=true, integer?:boolean):Uint8Array|Float32Array {
		const arrayBuffer = shared ? new ArrayBuffer(size  * 4) : new ArrayBuffer( size * 4)
	...
	static allocateMatrix(size:number, identity:boolean, shared=true):Float32Array {
		const m = new Float32Array(shared ? new ArrayBuffer(size * size * 4) : new ArrayBuffer(size * size * 4))
		if (identity) {
			let row = 0, column = 0
