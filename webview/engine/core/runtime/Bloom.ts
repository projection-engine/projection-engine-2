...
export default class Bloom {
	static #upSample(fbo: Framebuffer, context: WebGL2RenderingContext, nextSampler: WebGLTexture, blurredSampler: WebGLTexture) {
		...
		context.uniform1f(upSamplingShaderUniforms.sampleScale, ProjectionEngine.Engine.CameraAPI.bloomOffset)
	...
	static execute() {
		...
		if (!ProjectionEngine.Engine.CameraAPI.bloom)
			return
		...
		context.uniform1f(StaticShaders.bloomUniforms.threshold, ProjectionEngine.Engine.CameraAPI.bloomThreshold)
		StaticMeshes.drawQuad()
		...
		for (let i = 0; i < downscale.length; i++) {
			...
			context.uniform1i(StaticShaders.gaussianUniforms.samples, ProjectionEngine.Engine.CameraAPI.bloomQuality)
