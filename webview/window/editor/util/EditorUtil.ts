    ...
    static focusOnCamera(cameraTarget) {
        ...
        if (!focused || isCamera && cameraTarget.id !== focused) {
            ...
            if (current && current.cameraComponent) {
                EditorUtil.executionService.cameraSerialization = EditorUtil.engine.camera.serializeState()
                CameraTracker.stopTracking()
                EditorUtil.engine.camera.updateViewTarget(current)
                EditorUtil.settingsStore.updateStore({focusedCamera: current.id})
            }
        } else {
            EditorUtil.engine.camera.restoreState(EditorUtil.executionService.cameraSerialization)
